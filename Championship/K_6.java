...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        for (int left = aM; left >= 0; left--) {
            ...
            int currentD = minDown;
            int currentU = minUp;
            for (int right = aM; right < m; right++) {
                if (a[aN][right] != 'A' && a[aN][right] != '.') {
                    break;
                }
                currentU = Math.min(currentU, up[right]);
                currentD = Math.min(currentD, down[right]);
                int currentS = (right - left + 1) * (currentU + currentD + 1);
                if (maxS < currentS) {
                    maxS = currentS;
                    x1 = left;
                    x2 = right;
    ...
    public static char[][] stretching(char [][] a, int f1, int f2, int f3, int f4, int f5, int f6,boolean x){
        if(x) {
            for (int j = f1; j < f2; j++) {
                char c = ' ';
                for (int i = f3; i < f4; i++) {
